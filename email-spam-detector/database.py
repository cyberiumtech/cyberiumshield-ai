...
def save_scan(result):
    ...
    try:
        with database.cursor() as cursor:
            cursor.execute('''INSERT INTO email_spam_scans(sender,subject,verdict,score,confidence,model_name,model_version,result,scanned_at)
                VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)''', (result.get('sender'), result.get('subject'), result['verdict'],
                result['score'], result.get('confidence'), result.get('model'), result.get('model_version'),
