...
/* Sidebar surface color — must match the navbar background */
...
export function Sidebar({ activePath, mobile = false }: { activePath: string; mobile?: boolean }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className={mobile ? 'space-y-3' : isCollapsed ? 'space-y-1' : 'space-y-4'}>
          {sections.map((section, sectionIndex) => {
            ...
            // Hairline between sections when collapsed (no labels to separate them)
            ...
            return (
              <div key={section.title}>
                ...
                {/* Section label with horizontal rule (expanded only) */}
                {!mobile && !isCollapsed && (
                  ...
                  {items.map(it => {
                    ...
                    return (
                      <li key={it.path} className="relative">
                        ...
                        >
                          {/* Active indicator — thin cyan bar on the left edge */}
                          {active && (
                            <span
                              aria-hidden="true"
      ...
      {/* ═══ Bottom bar — pinned collapse toggle ═══ */}
      {!mobile && (
        <div
