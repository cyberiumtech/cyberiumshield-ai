...
const reportTypes = {
  'security-summary': {
    label: 'Security evidence summary',
    description: 'Posture metrics, source health, provenance, and validated findings.',
  },
  'finding-ledger': {
    label: 'Finding ledger',
    description: 'A chronological export of normalized findings and their source provenance.',
  },
  'source-audit': {
    label: 'Source audit trail',
    description: 'Connection state, freshness, record counts, and collection provenance.',
  },
} as const;
...
function createPdf(
  ...
  const addLines = (
    ...
    size = 9,
    gap = 4.5,
    color: [number, number, number] = [40, 51, 68]
  ) => {
  ...
  y = 44;
  addLines(
    `Generated: ${new Date(generatedAt).toLocaleString()}  |  Range: ${ranges[dateRange].label}`
  );
  addLines(
    `Evidence sync: ${data.lastSync ? new Date(data.lastSync).toLocaleString() : 'Not available'}`
  );
  addLines(
    'Integrity note: unavailable values are excluded and are never estimated.',
    9,
    5,
    [71, 85, 105]
  );
  ...
  if (reportType !== 'finding-ledger') {
    ...
    data.sources.forEach(source =>
      addLines(
        `${source.label} | ${source.status.toUpperCase()} | Records: ${source.recordCount ?? 'not exposed'} | Checked: ${source.updatedAt ? new Date(source.updatedAt).toLocaleString() : 'unavailable'} | ${source.provenance}`
      )
    );
  }
  if (reportType !== 'source-audit') {
    ...
    if (!findings.length)
      addLines('No findings with a usable timestamp were available in the selected range.');
    findings.forEach((finding, index) => {
      ...
      addLines(
        `${finding.source} | ${finding.status} | ${finding.timestamp ? new Date(finding.timestamp).toLocaleString() : 'time unavailable'}`,
        8,
        4,
        [71, 85, 105]
      );
...
export function ReportModal({
  ...
  const included =
    reportType === 'source-audit'
      ? `${data.sources.length} source attestations`
      : `${scopedFindings.length} findings and ${data.sources.length} source attestations`;
  ...
  const handleGenerate = (event: React.FormEvent) => {
    ...
    try {
      const report = {
        metadata: {
          title: reportTypes[reportType].label,
          reportType,
          dateRange,
          dateRangeLabel: ranges[dateRange].label,
          generatedAt,
          lastSuccessfulSync: data.lastSync,
          integrityNote: 'Unavailable values are excluded and never estimated.',
        },
      ...
      if (format === 'json') {
        ...
        const rows: unknown[][] = [
          ['Report', reportTypes[reportType].label],
          ['Generated at', generatedAt],
          ['Date range', ranges[dateRange].label],
          ['Last sync', data.lastSync ?? 'Unavailable'],
          ['Integrity note', 'Unavailable values are excluded and never estimated.'],
          [],
        ];
        if (reportType === 'security-summary')
          rows.push(
        ...
        if (reportType !== 'finding-ledger')
          rows.push(
            ['SOURCE HEALTH'],
            ['Source', 'Status', 'Last checked', 'Record count', 'Provenance'],
        ...
        if (reportType !== 'source-audit')
          rows.push(
            ['FINDINGS'],
            ['ID', 'Source', 'Severity', 'Status', 'Title', 'Detail', 'Timestamp'],
      ...
      onGenerated?.({
        name: reportTypes[reportType].label,
      ...
      toast.error('Unable to generate report', {
        description: 'The evidence remains unchanged. Please try again.',
      });
    } finally {
