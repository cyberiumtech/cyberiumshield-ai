...
const mapPoints = [
  ...
  { name: 'Mexico City', coordinates: [-99.13, 19.43] as [number, number], tone: '#f59e0b' },
  { name: 'Istanbul', coordinates: [28.97, 41.01] as [number, number], tone: '#fb7185' },
  { name: 'Seoul', coordinates: [126.98, 37.57] as [number, number], tone: '#22d3ee' },
  { name: 'Paris', coordinates: [2.35, 48.85] as [number, number], tone: '#a78bfa' },
  { name: 'Buenos Aires', coordinates: [-58.38, -34.61] as [number, number], tone: '#f59e0b' },
  { name: 'Cairo', coordinates: [31.24, 30.04] as [number, number], tone: '#fb7185' },
  { name: 'Bangkok', coordinates: [100.51, 13.75] as [number, number], tone: '#22d3ee' },
  { name: 'Dubai', coordinates: [55.27, 25.2] as [number, number], tone: '#a78bfa' },
  { name: 'Lagos', coordinates: [3.38, 6.52] as [number, number], tone: '#f59e0b' },
  { name: 'Jakarta', coordinates: [106.83, -6.21] as [number, number], tone: '#fb7185' },
  { name: 'Madrid', coordinates: [-3.7, 40.42] as [number, number], tone: '#22d3ee' },
  { name: 'Chicago', coordinates: [-87.63, 41.88] as [number, number], tone: '#a78bfa' },
  { name: 'Karachi', coordinates: [67.01, 24.86] as [number, number], tone: '#f59e0b' },
  { name: 'Lima', coordinates: [-77.03, -12.04] as [number, number], tone: '#fb7185' },
  { name: 'Tokyo', coordinates: [139.69, 35.69] as [number, number], tone: '#22d3ee' },
  { name: 'New York', coordinates: [-74.01, 40.71] as [number, number], tone: '#a78bfa' },
  { name: 'Singapore', coordinates: [103.82, 1.35] as [number, number], tone: '#f59e0b' },
];
  ...
  return (
    <div className="relative mx-auto w-full min-w-0 max-w-[1640px] pb-8 text-slate-200">
      ...
      <header className="mb-5 flex flex-col justify-between gap-5 border-b border-white/[0.08] pb-5 xl:flex-row xl:items-end">
        <div className="min-w-0"><div className="mb-3 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-cyan-300 sm:text-[11px] sm:tracking-[0.22em]"><ShieldCheck className="h-4 w-4 shrink-0" /> cyber Shield / Command Center</div><h1 className="max-w-[340px] text-[26px] font-semibold leading-tight tracking-[-0.035em] text-white sm:max-w-none sm:text-4xl">Security Operations Overview</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-400">Live posture across detection, infrastructure exposure, traffic and global threat intelligence.</p></div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="min-w-0 border-l border-white/10 px-4"><p className="font-mono text-sm text-slate-200">{formatDate(now.toISOString())}</p><p className="mt-1 text-xs uppercase tracking-[0.1em] text-slate-400">{now.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })} · Local time</p></div>
          <div className="min-w-0 border-l border-white/10 px-4"><p className={`flex items-center gap-2 text-sm font-medium ${sourceTone}`}><span className="relative flex h-2 w-2"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-current opacity-40" /><span className="relative inline-flex h-2 w-2 rounded-full bg-current" /></span>{sourceSummary}</p><p className="mt-1 text-xs uppercase tracking-[0.1em] text-slate-400">{data.summary.online}/6 online · {data.summary.offline} offline</p></div>
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(310px,.68fr)]">
        ...
        <Surface className="relative flex min-h-[480px] flex-col overflow-hidden p-6">
          ...
          <div className="relative mt-3 flex min-w-0 flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between"><span className="text-sm font-semibold text-slate-200">{posture.label}</span><span tabIndex={0} className="text-xs text-slate-400 outline-none focus:text-cyan-200" title="Risk is derived from available network status, vulnerability severity, local scan verdicts and recent CISA KEV additions." aria-label="Risk is derived from available network status, vulnerability severity, local scan verdicts and recent CISA KEV additions.">Available signals only</span></div>
      ...
      <div className="mt-5 grid min-w-0 items-start gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(320px,.7fr)]">
        <Surface className="overflow-hidden"><div className="flex items-center justify-between border-b border-white/[0.08] px-5 py-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-rose-300">Prioritized queue</p><h2 className="mt-1 text-lg font-semibold text-white">Live attention feed</h2></div><Activity className="h-5 w-5 text-cyan-300" /></div>
          {activities.length ? <div className="divide-y divide-white/[0.06]">{activities.map(item => { const tone = item.severity === 'critical' ? 'bg-rose-400' : item.severity === 'warning' ? 'bg-amber-300' : item.severity === 'ok' ? 'bg-emerald-300' : 'bg-cyan-300'; return <Link key={item.id} to={item.route} className="group grid min-w-0 gap-3 px-5 py-4 transition hover:bg-white/[0.025] focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-300 sm:grid-cols-[12px_minmax(0,1fr)_150px_18px] sm:items-center"><span className={`h-1.5 w-1.5 rounded-full ${tone}`} /><div className="min-w-0"><p className="truncate text-sm font-medium text-slate-200">{item.title}</p><p className="mt-1 truncate text-xs text-slate-400">{item.detail}</p></div><div className="text-left sm:text-right"><p className="text-xs font-medium text-slate-300">{item.source}</p><p className="mt-1 font-mono text-xs text-slate-400" title={formatDate(item.time, true)}>{relativeTime(item.time)}</p></div><ArrowRight className="hidden h-4 w-4 text-slate-500 transition group-hover:translate-x-0.5 group-hover:text-cyan-300 sm:block" /></Link>; })}</div> : <div className="grid min-h-72 place-items-center px-6 text-center"><div><CloudOff className="mx-auto h-8 w-8 text-slate-500" /><p className="mt-3 text-sm font-medium text-slate-300">No activity yet</p><p className="mt-1 text-xs text-slate-400">Run a scan or connect a service to populate this feed.</p></div></div>}
        </Surface>
