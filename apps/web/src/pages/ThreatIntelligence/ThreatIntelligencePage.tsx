...
function ServiceVisibility({
  ...
  return (
    <section
      ...
      aria-labelledby="service-visibility-title"
    >
      <header className="flex items-center justify-between gap-3 border-b border-slate-800 px-4 py-3 sm:px-5">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Provider availability can change the evidence returned.
          </p>
        </div>
      ...
      {isLoading ? (
        <div
          className="grid grid-cols-2 gap-px bg-slate-800 lg:grid-cols-4"
          aria-label="Loading provider status"
        >
          {[1, 2, 3, 4].map(item => (
            <div key={item} className="h-24 animate-pulse bg-[#0F1729]" />
          ))}
        </div>
      ) : isError && !health && !feeds ? (
        <p className="px-5 py-5 text-sm text-rose-200">
        ...
        <div className="grid grid-cols-2 gap-px bg-slate-800 lg:grid-cols-4">
          {items.map(({ label, value, note, available, icon: Icon }) => (
            <article key={label} className="min-w-0 bg-[#0F1729] px-4 py-3.5">
              ...
              <p className="mt-1 truncate text-[10px] text-slate-500" title={note}>
                {note}
              </p>
            </article>
          ))}
...
function ResultWorkspace({ result }: { result: IndicatorResult }) {
  ...
  return (
    <section
      className={`${panel} overflow-hidden rounded-[14px]`}
      aria-labelledby="investigation-result-title"
    >
      <header className="grid gap-5 border-b border-slate-800 px-4 py-5 sm:grid-cols-[auto_minmax(0,1fr)] sm:px-6">
        <div
          className="relative flex h-28 w-28 shrink-0 items-center justify-center rounded-full"
          style={{
            background: `conic-gradient(${tone.ring} ${result.riskScore}%, #1e293b ${result.riskScore}% 100%)`,
          }}
        ...
        <div className="min-w-0 self-center">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`border px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.16em] ${tone.className}`}
            >
          ...
          <h2
            ...
            className="mt-3 break-all font-mono text-base font-bold leading-6 text-slate-100"
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="font-mono text-[9px] uppercase tracking-wider opacity-75">
                  {result.coverage.sourcesQueried}/{result.coverage.sourcesExpected} reputation
                  sources completed
                </span>
              </div>
          ...
          {result.coverage.providers.length > 0 && (
            <ul
              ...
              aria-label="Provider contribution status"
            >
      ...
      {result.providerErrors.length > 0 && (
        <aside
          className="flex items-start gap-2.5 border-b border-amber-400/20 bg-amber-400/[.06] px-4 py-3 text-xs leading-5 text-amber-100 sm:px-6"
          aria-label="Partial provider warnings"
        >
        ...
        <div className="space-y-6 bg-[#0F1729] px-4 py-5 sm:px-6">
          <section aria-labelledby="reasons-title">
            <div className="flex items-center gap-2">
              ...
              <h3 id="reasons-title" className="text-sm font-semibold text-slate-100">
                Assessment reasons
              </h3>
            </div>
            {result.reasons.length ? (
              <ol className="mt-3 space-y-3">
                {result.reasons.map((reason, index) => (
                  ...
                  >
                    <span className="mt-0.5 font-mono text-[10px] text-cyan-300">
          ...
          <section aria-labelledby="evidence-title">
            ...
            {Object.keys(result.evidence).length ? (
              ...
              <p className="mt-2 text-sm text-slate-500">
                No provider evidence facts were available.
              </p>
            )}
          </section>
        </div>
        ...
        <div className="space-y-4 bg-[#0B1120] px-4 py-5 sm:px-6">
          ...
          {cisaKEV && (
            <section className="border-l-2 border-rose-400 bg-rose-400/[.055] px-4 py-3">
              <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-rose-300">
          ...
          {nvd && (
            <section className="border border-slate-800 bg-[#0F1729] px-4 py-3">
              ...
              {nvd.vector && (
                <p className="mt-3 break-all border-l border-slate-700 pl-3 font-mono text-[10px] leading-5 text-slate-400">
                  {nvd.vector}
                </p>
              )}
                  ...
                  {nvd.references.slice(0, 4).map((reference, index) => {
                    ...
                    return href ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer noopener"
                        className="inline-flex items-center gap-1 text-xs font-semibold text-cyan-300 hover:text-cyan-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"
                      >
                        Reference {index + 1}
            ...
            <section className="border border-slate-800 bg-[#0F1729] px-4 py-3">
              ...
              <p className="mt-2 text-xs text-slate-500">
                {threatFox.configured
                  ? `Provider status: ${threatFox.status || 'queried'}`
                  : 'ThreatFox is not configured on this service.'}
              </p>
              {matches.slice(0, 3).map((match, index) => (
                <dl
                  key={index}
                  className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1 border-t border-slate-800 pt-3 text-[10px]"
                >
          ...
          {dns && (
            <section className="border border-slate-800 bg-[#0F1729] px-4 py-3">
              ...
              {dns.addresses.length > 0 ? (
                ...
                <p className="mt-2 text-xs text-slate-500">
                  No addresses were returned by the service resolver.
                </p>
              )}
            </section>
          )}
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="font-mono text-xs text-slate-300">
                  {typeof mlProbability === 'number'
                    ? `${(mlProbability * 100).toFixed(1)}% model probability`
                    : 'No probability'}
                </span>
              </div>
          ...
          {!cisaKEV && !nvd && !threatFox && !dns && !urlFeatures && (
            <p className="border border-dashed border-slate-700 px-4 py-6 text-sm leading-6 text-slate-500">
      ...
      <footer className="flex flex-col gap-2 border-t border-slate-800 bg-[#0F1729] px-4 py-3 text-xs leading-5 text-slate-500 sm:flex-row sm:items-start sm:justify-between sm:px-6">
        <p className="max-w-3xl">
          ...
          whether a verdict is supported; no-match or missing evidence is never proof of safety.
        </p>
        <span
...
function InvestigationHistory({
  ...
  return (
    <section
      className={`${panel} overflow-hidden rounded-[14px]`}
      aria-labelledby="investigation-history-title"
    >
      <header className="flex items-start justify-between gap-4 border-b border-slate-800 px-4 py-4 sm:px-5">
        <div>
          <div className="flex items-center gap-2">
            <History className="h-4 w-4 text-cyan-300" />
            <h2 id="investigation-history-title" className="text-sm font-semibold text-slate-100">
              Recent investigations
            </h2>
          </div>
          <p className="mt-1 text-xs text-slate-500">
            Select a stored result to inspect it without querying providers again.
          </p>
        </div>
        <button
          type="button"
          className={`${button} min-h-9 shrink-0 px-2.5`}
          onClick={onRefresh}
      ...
      {isLoading ? (
        <div className="space-y-px bg-slate-800" aria-label="Loading recent investigations">
          {[1, 2, 3].map(item => (
            <div key={item} className="h-14 animate-pulse bg-[#0F1729]" />
          ))}
        </div>
      ) : error ? (
        <div className="px-5 py-8 text-center">
          <FileWarning className="mx-auto h-5 w-5 text-rose-300" />
          <p className="mt-2 text-sm font-semibold text-slate-200">History could not be loaded</p>
          <p className="mt-1 text-xs text-slate-500">{errorMessage(error)}</p>
        </div>
      ) : records.length === 0 ? (
        <div className="px-5 py-9 text-center">
          <History className="mx-auto h-6 w-6 text-slate-600" />
          <p className="mt-3 text-sm font-semibold text-slate-300">No stored investigations yet</p>
          <p className="mt-1 text-xs text-slate-500">
            Completed checks will appear here, newest first.
          </p>
        </div>
      ) : (
        <>
                ...
                {records.map((record, index) => {
                  ...
                  return (
                    <tr
                      ...
                      className="hover:bg-cyan-300/[.025]"
                    >
                      <td className="max-w-md px-4 py-3">
                        <button
                          type="button"
                          onClick={() => onSelect(record)}
                          className="block w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"
                        >
                          <span className="block text-[9px] font-bold uppercase tracking-wider text-cyan-300">
                            {record.indicatorType}
                          </span>
                          <span
                            className="mt-1 block truncate font-mono text-xs text-slate-200"
                      ...
                      <td className="px-4 py-3 font-mono text-sm font-bold text-slate-100">
                        {record.riskScore}
                      </td>
            ...
            {records.map((record, index) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => onSelect(record)}
                  className="block w-full px-4 py-4 text-left hover:bg-cyan-300/[.025] focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-300"
                >
                  ...
                  <span
                    className="mt-2 block truncate font-mono text-xs text-slate-200"
                    title={record.indicator}
                  >
                    {record.indicator}
                  </span>
  ...
  return (
    <div className="relative space-y-5">
      ...
      <header className="relative flex flex-col gap-4 border-b border-slate-800 pb-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div
          className={`inline-flex min-h-10 items-center gap-2 self-start border px-3 text-xs font-semibold lg:self-auto ${serviceState.className}`}
          aria-live="polite"
        >
      ...
      <nav
        className="relative flex w-full border-b border-slate-800"
      ...
      {activeTab === 'investigations' && (
        <div className="relative space-y-5">
          <section
            className={`${panel} overflow-hidden rounded-[14px]`}
            aria-labelledby="indicator-composer-title"
          >
            <div className="grid gap-px bg-slate-800 lg:grid-cols-[minmax(0,1fr)_240px]">
              <div className="bg-[#0F1729] px-4 py-5 sm:px-6">
                ...
                <form className="mt-4" onSubmit={submitIndicator} noValidate>
                  <label
                    htmlFor="threat-indicator"
                    className="mb-2 block text-[10px] font-semibold uppercase tracking-[0.15em] text-slate-400"
                  >
                    Indicator value
                  </label>
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <div className="relative min-w-0 flex-1">
                      ...
                      <input
                        ...
                        className={`${control} h-12 w-full pl-10 font-mono`}
                    ...
                    <button
                      type="submit"
                      className={`${button} h-12 border-cyan-400/35 bg-cyan-400/[.08] px-5 text-cyan-100`}
                      disabled={lookup.isPending || !indicator.trim()}
                    >
                      {lookup.isPending ? (
                        <>
                          <RefreshCw className="h-4 w-4 animate-spin" /> Checking providers
                        </>
                      ) : (
                        <>
                          <Search className="h-4 w-4" /> Run investigation
                        </>
                      )}
                    </button>
                    {lookup.isPending && (
                  ...
                  <div
                    id="indicator-help"
                    className="mt-3 flex flex-wrap items-center gap-2 text-[10px] text-slate-500"
                  >
                    ...
                    {['CVE-2024-3094', '8.8.8.8', 'example.com', 'https://example.com/login'].map(
                      example => (
                        <button
                          ...
                          disabled={lookup.isPending}
                          className="border border-slate-700 bg-[#0B1120] px-2 py-1 font-mono text-slate-400 transition hover:border-cyan-400/40 hover:text-cyan-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 disabled:opacity-50"
                        >
                          {example}
                        </button>
                      )
                  ...
                  <div id="indicator-error" className="mt-3 min-h-5 text-xs" aria-live="assertive">
                    ...
                    {lookup.isPending && (
                      <p className="text-cyan-300">
                        Querying applicable providers. Slow upstream services may take up to 40
                        seconds.
                      </p>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <ol className="mt-3 space-y-3 text-xs text-slate-400">
                    <li className="flex gap-2">
                      <span className="font-mono text-cyan-300">01</span>Classify indicator
                    </li>
                    <li className="flex gap-2">
                      <span className="font-mono text-cyan-300">02</span>Collect provider evidence
                    </li>
                    <li className="flex gap-2">
                      <span className="font-mono text-cyan-300">03</span>Score deterministic signals
                    </li>
                  </ol>
                </div>
                <p className="mt-5 border-t border-slate-800 pt-3 text-[10px] leading-4 text-slate-600">
                  Provider failures are reported as partial-data warnings and do not erase
                  successful evidence.
                </p>
              </aside>
            </div>
          ...
          <div aria-live="polite">
            {activeResult ? (
              ...
              <section className="border border-dashed border-slate-700 bg-[#0F1729]/50 px-5 py-8 text-center">
          ...
          {query.isLoading ? (
            ...
            >
              ...
              <p className="mx-auto mt-2 max-w-xl text-xs leading-5 text-slate-500">
                No fabricated or bundled vulnerability records are shown. Access the official
                catalog directly or retry when the source is reachable.
              </p>
